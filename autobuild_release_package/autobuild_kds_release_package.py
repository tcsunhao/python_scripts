...
# python autobuild_kds_release_package.py -n frdmk82f -m debug
# python autobuild_kds_release_package.py -n frdmk82f -m release
# python autobuild_kds_release_package.py -n frdmk82f -m all
...
def _run_command(kds_path, filename_path, proj_name, build_mode):
    ...
    if build_mode == 'debug':
        ...
        print '%s %s make %s' % (kds_path, filename_path, build_mode)
        sys.stdout.flush()

        kds_debug_tmp_log_path = os.getcwd() + '\\kds_debug_tmp_log.txt'
        ...
        "%s" --launcher.suppressErrors -nosplash -application "org.eclipse.cdt.managedbuilder.core.headlessbuild" -build "%s" -import "%s" -data "%s" >> %s 2>&1 ' % (
                ...
                proj_name + '/' + build_mode,
                import_path + '/',
                './',
                kds_debug_tmp_log_path
                )
        proj_name_withmode = proj_name + ' ' + build_mode
        debug_task = subprocess.Popen(kds_build_cmd, 0, stdin=None, stdout=None, stderr=None, shell = True)
        debug_ret = debug_task.wait()
        
        # If the project build failed
        if debug_ret != 0 :
            projectbuild_fail_number += 1
            kds_log_dic['projectbuild_fail_number'] = projectbuild_fail_number
            __error_log_filter(kds_debug_tmp_log_path, file_fail, proj_name_withmode)
            print 78*'X'
            print proj_name_withmode + ' ' + 'build failed' + '\n'
        # If the project build passed, find the warnings
        else : 
            has_warning = __warning_log_filter(kds_debug_tmp_log_path, file_pass_withwarning, proj_name_withmode)
            if has_warning == 1:
                projectbuild_warning_number += 1
                kds_log_dic['projectbuild_warning_number'] = projectbuild_warning_number
                print 78*'W'
                print proj_name_withmode + ' ' + 'build pass with warnings' + '\n'
            else:
                projectbuild_pass_number += 1
                kds_log_dic['projectbuild_pass_number'] = projectbuild_pass_number
        ...
        os.remove(kds_debug_tmp_log_path)
    elif build_mode == 'release':
        
        print '%s %s make %s' % (kds_path, filename_path, build_mode)
        sys.stdout.flush()

        kds_release_tmp_log_path = os.getcwd() + '\\kds_release_tmp_log.txt'
        kds_build_cmd = 'set path=%s;%s;%s && \
        "%s" --launcher.suppressErrors -nosplash -application "org.eclipse.cdt.managedbuilder.core.headlessbuild" -build "%s" -import "%s" -data "%s" >> %s 2>&1 ' % (
                kds_path + '/bin',
                kds_path + '/toolchain/bin',
                ...
                import_path + '/',
                './',
                kds_release_tmp_log_path
                )
        proj_name_withmode = proj_name + ' ' + build_mode
        release_task = subprocess.Popen(kds_build_cmd, 0, stdin=None, stdout=None, stderr=None, shell = True)
        ...
        if release_ret != 0 :
            projectbuild_fail_number += 1
            kds_log_dic['projectbuild_fail_number'] = projectbuild_fail_number
            __error_log_filter(kds_release_tmp_log_path, file_fail, proj_name_withmode)
            print 78*'X'
            print proj_name_withmode + ' ' + 'build failed' + '\n'
        # If the project build passed, find the warnings
        else : 
            has_warning = __warning_log_filter(kds_release_tmp_log_path, file_pass_withwarning, proj_name_withmode)
            if has_warning == 1:
                projectbuild_warning_number += 1
                kds_log_dic['projectbuild_warning_number'] = projectbuild_warning_number
                print 78*'W'
        ...
        os.remove(kds_release_tmp_log_path) 
        ...
        kds_cmd_release = 'set path=%s;%s;%s && \
        "%s" --launcher.suppressErrors -nosplash -application "org.eclipse.cdt.managedbuilder.core.headlessbuild" -build "%s" -import "%s" -data "%s" >> %s 2>&1 ' % (
                kds_path + '/bin',
                kds_path + '/toolchain/bin',
                '%SystemRoot%\system32;%SystemRoot%',
        ...
        debug_task = subprocess.Popen(kds_cmd_debug, 0, stdin=None, stdout=None, stderr=None, shell=True)
        debug_ret = debug_task.wait()
        release_task = subprocess.Popen(kds_cmd_release, 0, stdin=None, stdout=None, stderr=None, shell=True)
        release_ret = release_task.wait()
...
if __name__ == '__main__':
    ...
    if the_args.buildname == None:
        print 'You need give a name for this build, the name can distinguish itself from next time build, such as python autobuild_kds_release_package -n frdmkxx -m debug'
        sys.exit()
    else:
        pass
    ...
    if os.path.exists(log_file_path):
        ...
        if os.path.exists(log_file_path + '\\%s' % file_proj_num_statistics):
            ...
            kds_log_dic_tmp = yaml.load(f)
            if kds_log_dic_tmp:
                kds_log_dic = kds_log_dic_tmp.copy()
                if kds_log_dic.has_key('projectbuild_pass_number'):
                    projectbuild_pass_number = projectbuild_pass_number + kds_log_dic['projectbuild_pass_number']
                if kds_log_dic.has_key('projectbuild_warning_number'):
                    projectbuild_warning_number = projectbuild_warning_number + kds_log_dic['projectbuild_warning_number']
                if kds_log_dic.has_key('projectbuild_fail_number'):
                    projectbuild_fail_number = projectbuild_fail_number + kds_log_dic['projectbuild_fail_number']
                if kds_log_dic.has_key('progressbar_counter'):
                    progressbar_counter = progressbar_counter + kds_log_dic['progressbar_counter']
            f.close()          
    else:
            ...
            if re.search(r'\.project', filename_path):
                if filename_path.find('kds') != -1:
                    ...
                    if has_build == 0:
                        ...
                        _run_command(kds_bin_path, filename_path , proj_name, build_mode)
                        
                        progressbar_counter = progressbar_counter + 1
                        print pb(progressbar_counter)
                        sys.stdout.flush()
                        kds_log_dic['progressbar_counter'] = progressbar_counter
                        # Update the file_proj_num_statistics
                        f = open(file_proj_num_statistics,'w')
                        yaml.dump(kds_log_dic, f)
                        f.close()
    ...
    __output_log(kds_log_dic, file_pass_withoutwarning, file_pass_withwarning, file_fail, path_log_file)
