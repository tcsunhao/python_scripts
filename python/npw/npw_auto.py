...
class KSDKPackageGroup():
    ...
    def search_SDK_zip_packages(self, root_folder, level):
        ...
        for item in os.listdir(root_folder):
            ...
            if os.path.isfile(ab_path):
                ...
                if BACKUP_FOLDER not in ab_path:
                    self.search_SDK_zip_packages(ab_path, level + 1)


    def unzip_SDK_packages(self, root_folder, force_replace):
...
if __name__ == '__main__':
    ...
    arguments = Utility.read_options()
    ...
    if arguments.unzip_packages:
        ksdk_pkg_group.search_SDK_zip_packages(arguments.parent_path, 1)
        ksdk_pkg_group.unzip_SDK_packages(arguments.parent_path, arguments.force_replace)
    ...
    if arguments.build:
        npw.scan_packages()
        npw.build()
        npw.report_summary(False)
        npw.report_summary(True) # use to generate failure case log
